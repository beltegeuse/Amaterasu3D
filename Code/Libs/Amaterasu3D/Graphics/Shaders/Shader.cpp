...
Shader::Shader(ShaderUnit * VertexShader, ShaderUnit * FragmentShader,
		ShaderUnit * GeometryShader) :
		m_VertexShader(VertexShader), m_FragementShader(FragmentShader), m_GeometryShader(
				GeometryShader), m_FBO(0), m_IsLink(false)
{
	// Program creation
...
Shader::~Shader()
{
	std::cout << "[DEBUG] Delete shader : " << m_ProgramID << " ..."
			<< std::endl;
	// Free OpenGL
...
void Shader::SetGeometryShaderParameters(GLenum inputMode, GLenum outputMode,
		int output)
{
...
{
	for (MapAttributs::iterator it = m_attributs_bind.begin();
			it != m_attributs_bind.end(); it++)
			{
...
{
	for (MapMatrix::iterator it = m_matrix_bind.begin();
			it != m_matrix_bind.end(); ++it)
			{
...
TShaderPtr CShaderManager::LoadShader(const std::string& filename)
{
	...
	if (Resource == NULL)
	{
		Resource = CMediaManager::Instance().LoadMediaFromFile<Shader>(
				filename);
...
Shader* CShaderManager::CreateShader(ShaderUnit * VertexShader,
...
Shader* CShaderManager::loadfromFile(const char* vertexFile,
		const char* fragmentFile, ShaderType type,
		const ShaderCompilerConfig& config)
{
	return CreateShader(new ShaderUnit(vertexFile, VERTEX_SHADER, config),
			new ShaderUnit(fragmentFile, FRAGMENT_SHADER, config), 0, type);
}

Shader* CShaderManager::loadfromFile(const char* vertexFile,
		const char* fragmentFile, const char* geometryFile, ShaderType type,
		const ShaderCompilerConfig& config)
{
	return CreateShader(new ShaderUnit(vertexFile, VERTEX_SHADER, config),
			new ShaderUnit(fragmentFile, FRAGMENT_SHADER, config),
			new ShaderUnit(geometryFile, GEOMETRY_SHADER, config), type);
}
