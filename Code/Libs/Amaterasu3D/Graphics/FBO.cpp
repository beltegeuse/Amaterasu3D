...
FBO::FBO(const Math::TVector2I& size,
		...
		m_DepthType(type), m_DepthID(0), m_SizeBuffers(size), m_IsActivated(
				false), m_BuffersParams(buffers), m_DepthParams(paramDepth)
{
			...
			buffers.begin(); it != buffers.end(); it++)
	{
		...
		glFramebufferTexture2D(GL_FRAMEBUFFER,
				GL_COLOR_ATTACHMENT0 + it->second.Attachment, GL_TEXTURE_2D,
				m_ColoredBuffers[it->first]->getIdTex(), 0);
...
void FBO::SetSize(const Math::TVector2I& size)
{
	...
	for (std::map<std::string, Texture*>::iterator it = m_ColoredBuffers.begin(); it != m_ColoredBuffers.end(); ++it)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, params.InternalFormat, size.x, size.y, 0,
				params.ExternalFormat, params.Precision, 0);
	...
	if (m_DepthType != FBODEPTH_NONE)
	{
		if (m_DepthType == FBODEPTH_TEXTURE)
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, m_DepthParams.InternalFormat, size.x,
					size.y, 0, m_DepthParams.ExternalFormat,
					m_DepthParams.Precision, 0);
...
void FBO::UnBind()
{
	...
	for (std::map<std::string, FBOTextureBufferParam>::iterator it =
			m_BuffersParams.begin(); it != m_BuffersParams.end(); ++it)
	{
