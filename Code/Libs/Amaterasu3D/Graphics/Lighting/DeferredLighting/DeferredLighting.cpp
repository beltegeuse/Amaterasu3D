...
void DeferredLighting::PointLightPass()
{
	...
	m_FBO_graphics->GetTexture("Specular")->activateMultiTex(
			CUSTOM_TEXTURE + 1);
	m_FBO_graphics->GetTexture("Normal")->activateMultiTex(CUSTOM_TEXTURE + 2);
	m_FBO_graphics->GetTexture("Position")->activateMultiTex(
			CUSTOM_TEXTURE + 3);
	// Go light pass
	...
	for (int i = 0; i < m_points_lights.size(); i++)
	{
		...
		m_point_light_shader->SetUniform1f("LightRaduis",
				m_points_lights[i].LightRaduis);
		m_point_light_shader->SetUniform1f("LightIntensity",
				m_points_lights[i].LightIntensity);
		m_point_light_shader->SetUniformColor("LightColor",
				m_points_lights[i].LightColor);
		m_point_light_shader->SetUniformVector("LightPosition",
				m_points_lights[i].Position);
	...
	m_FBO_graphics->GetTexture("Normal")->desactivateMultiTex(
			CUSTOM_TEXTURE + 2);
	m_FBO_graphics->GetTexture("Position")->desactivateMultiTex(
			CUSTOM_TEXTURE + 3);
}
