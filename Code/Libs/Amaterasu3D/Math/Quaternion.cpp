...
CQuaternion CQuaternion::slerp(const CQuaternion &q1, const CQuaternion &q2,
		float t)
{
	CQuaternion q3;
	...
	if (dot < 0.95f)
	{
		...
		return (q1 * sinf(angle * (1 - t)) + q3 * sinf(angle * t))
				* (1.0 / sinf(angle));
	}
	else
...
CQuaternion CQuaternion::slerpNoInvert(const CQuaternion &q1,
		const CQuaternion &q2, float t)
{
	...
	{
		float angle = acosf(dot);
		return (q1 * sinf(angle * (1 - t)) + q2 * sinf(angle * t))
				* (1.0 / sinf(angle));
	}
	else
		// if the angle is small, use linear interpolation
