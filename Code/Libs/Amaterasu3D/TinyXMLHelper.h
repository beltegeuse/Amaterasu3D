...
struct CTinyXMLAttributException: public CTinyXMLException
{
			...
			CTinyXMLException("")
	{
		if (tinyXMLerror == TIXML_WRONG_TYPE)
			m_Message += " can't get the value of the attribute : "
					+ attributName;
...
template<typename T> inline void TinyXMLGetAttributeValue(TiXmlElement* element,
		const std::string nameAttribute, T* value)
{
