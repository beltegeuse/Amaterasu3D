...
DurandToneOperator::DurandToneOperator()
{
	m_BilateralShader = CShaderManager::Instance().LoadShader(
			"BilateralFiltering.shader");
	m_DurandShader = CShaderManager::Instance().LoadShader(
			"DurandToneMapping.shader");
}
...
//	std::cout << "[DEBUG] Half : " << HalfSize << "\n";
	...
	HDRBuffer->activateMultiTex(CUSTOM_TEXTURE + 0);
	m_BilateralShader->GetFBO()->GetTexture("Result")->activateMultiTex(
			CUSTOM_TEXTURE + 1);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0, 0.0);
	glVertex2f(-1.0, -1.0);
	...
	glEnd();
	HDRBuffer->desactivateMultiTex(CUSTOM_TEXTURE + 0);
	m_BilateralShader->GetFBO()->GetTexture("Result")->desactivateMultiTex(
			CUSTOM_TEXTURE + 1);
	m_DurandShader->End();
}
